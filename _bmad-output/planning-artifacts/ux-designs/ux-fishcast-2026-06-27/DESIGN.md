...
- **Status badge** (`{components.status-badge}`) — pill, three variants (safe / watch / exceeded). Always paired with a text label ("Under threshold" / "Approaching" / "Exceeded"), never color alone. See [`mockups/key-manager-view.html`](mockups/key-manager-view.html), [`mockups/key-fishery-picker.html`](mockups/key-fishery-picker.html).
- **Mode toggle** (`{components.mode-toggle}`) — segmented control, two options (Manager / Angler), lives in the header, persists across fishery navigation. See any mock's header.
- **Harvest number trio** (`{components.harvest-number-trio}`) — the visual contract for the three harvest figures: reported is a hollow/outline numeral, estimated is solid, projected is set with a dashed underline and shown with its uncertainty range. The three must always appear together with this same treatment, never re-skinned per-screen. See [`mockups/key-angler-view.html`](mockups/key-angler-view.html), [`mockups/key-manager-view.html`](mockups/key-manager-view.html).
- **Projection band chart** (`{components.projection-band-chart}`) — the highest-stakes visual in the product: a line in `{colors.data-projected}` with a low-opacity (`band-opacity: 0.18`) fill of the same color marking the uncertainty range, plotted against a muted axis (`{colors.text-muted}`) and faint gridlines (`{colors.surface-sunken}`). Hovering/tapping a point shows a `{colors.primary}` marker and reveals the exact value + range as text — the chart is never the only way to read a number. See [`mockups/key-angler-view.html`](mockups/key-angler-view.html).
- **Fishery/season picker** (`{components.fishery-picker}`) — a combobox: `{rounded.sm}` input, `{rounded.sm}` dropdown rows. Recently-viewed rows get a `{colors.surface-muted}` background to visually separate them from the rest of the filtered list, no other treatment difference. See [`mockups/key-fishery-picker.html`](mockups/key-fishery-picker.html).
- **Share Summary view** (`{components.share-summary-view}`) — reuses the standard card grid on-screen with full `{colors.primary}` chrome; the printed/exported artifact strips to monochrome (`{colors.text}` on white) so it photocopies and prints cleanly for a co-manager briefing. See [`mockups/key-share-summary.html`](mockups/key-share-summary.html).
- **Accuracy strip** — no new tokens; it reuses `{components.harvest-number-trio}`'s `estimated` treatment for the confirmed-actual point and its `projected` treatment for the past projection's band, so a past-projection-vs-actual comparison reads with the same visual grammar a user already learned from the Angler/Manager views. See [`mockups/key-forecast-accuracy.html`](mockups/key-forecast-accuracy.html).
